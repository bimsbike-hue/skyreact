...
export type UploadedModel = {
  filename: string;
  storagePath: string; // e.g. print-store/<uid>/<ts>-file.stl
  publicUrl: string;   // requires bucket to be public (recommended for downloads)
};
 ...
 */
export async function uploadModelToSupabase(
  file: File,
  uid: string
  ...
  if (!ALLOWED_EXTS.includes(ext)) {
    throw new Error(
      `File type ".${ext}" is not allowed. Allowed: ${ALLOWED_EXTS.join(", ")}`
    );
  }
  ...
  if (error) {
    ...
    if (String(error.message).toLowerCase().includes("bucket not found")) {
      throw new Error(
        `Supabase bucket "${bucket}" not found. Create it in Supabase → Storage (name must match VITE_SUPABASE_BUCKET), then retry.`
      );
    }
    throw error;
  }
  ...
  if (!publicUrl) {
    throw new Error(
      `Upload succeeded but no public URL. Make sure the "${bucket}" bucket is set to Public, or implement signed URLs.`
    );
  }
